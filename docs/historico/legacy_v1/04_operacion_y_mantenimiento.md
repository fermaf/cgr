...
### Síntoma: iniciales agrupadas en cat_abogados (ej: "EMV APT")

- Interpretación: El parser de ingesta no separó por espacios.
- Solución: Actualizar `extractCommaSeparatedList` para incluir `\s` en el split y Regex de validación.
- Acción correctiva: Ejecutar script de saneamiento SQL y reprocesar afectados vía `/api/v1/dictamenes/:id/re-process`.
