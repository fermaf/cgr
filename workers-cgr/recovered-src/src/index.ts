...
function getRawSource2(raw) {
  return raw._source ?? raw.source ?? raw.raw_data ?? raw;
}
__name(getRawSource2, "getRawSource");
function extractDictamenId2(raw) {
  const source = getRawSource2(raw);
  const docId = typeof source.doc_id === "string" ? source.doc_id : null;
  const rawId = typeof raw._id === "string" ? raw._id : null;
  const id = typeof raw.id === "string" ? raw.id : null;
  return docId ?? rawId ?? id ?? null;
}
__name(extractDictamenId2, "extractDictamenId");
function extractVectorText(raw, enrichment) {
  const source = getRawSource2(raw);
  const candidates = [
...
__name(extractVectorText, "extractVectorText");
function countWords(value) {
  return value.trim().split(/\s+/).filter(Boolean).length;
}
__name(countWords, "countWords");
function getDocumentoCompleto(raw) {
  const source = getRawSource2(raw);
  const doc = source.documento_completo;
  return typeof doc === "string" && doc.trim().length > 0 ? doc : null;
}
__name(getDocumentoCompleto, "getDocumentoCompleto");
function formatCreatedAt(timeZone, date = /* @__PURE__ */ new Date()) {
...
__name(formatCreatedAt, "formatCreatedAt");
var STABLE_PAGE_THRESHOLD = 3;
var STABLE_PAGE_RATIO = 1;
function getTimeZone(env) {
  return env.APP_TIMEZONE ?? "America/Santiago";
}
__name(getTimeZone, "getTimeZone");
function getStableSettings(env) {
...
__name(getStableSettings, "getStableSettings");
...
function normalizeBooleanos2(input) {
  return {
...
__name(normalizeBooleanos2, "normalizeBooleanos");
...
__name(normalizeDescriptors, "normalizeDescriptors");
...
function buildPineconeMetadata(raw, enrichment, modelNamespace, timeZone) {
  const source = getRawSource2(raw);
...
__name(buildPineconeMetadata, "buildPineconeMetadata");
...
async function handleImportMongo(request, env) {
  if (env.IMPORT_TOKEN) {
    const token = request.headers.get("x-import-token");
    if (!token || token !== env.IMPORT_TOKEN) {
      return jsonResponse({ error: "unauthorized" }, { status: 401 });
  ...
  const ids = items.map((raw) => raw && typeof raw === "object" ? extractDictamenId2(raw) : null).filter((id) => Boolean(id));
  ...
  for (const raw of items) {
    if (!raw || typeof raw !== "object") continue;
    const record = raw;
    const dictamenId = extractDictamenId2(record);
    ...
    const createdAt = typeof record.creado_en === "string" ? record.creado_en : (/* @__PURE__ */ new Date()).toISOString();
...
__name(handleImportMongo, "handleImportMongo");
async function handleBackfillCanonical(request, env) {
  if (env.IMPORT_TOKEN) {
    const token = request.headers.get("x-import-token");
    if (!token || token !== env.IMPORT_TOKEN) {
      return jsonResponse({ error: "unauthorized" }, { status: 401 });
...
__name(handleBackfillCanonical, "handleBackfillCanonical");
async function handleBackfillDocumentoMissing(request, env) {
  if (env.IMPORT_TOKEN) {
    const token = request.headers.get("x-import-token");
    if (!token || token !== env.IMPORT_TOKEN) {
      return jsonResponse({ error: "unauthorized" }, { status: 401 });
...
__name(handleBackfillDocumentoMissing, "handleBackfillDocumentoMissing");
async function runBackfillCanonical(env, options) {
...
__name(runBackfillCanonical, "runBackfillCanonical");
async function runBackfillDocumentoMissing(env, limit) {
...
__name(runBackfillDocumentoMissing, "runBackfillDocumentoMissing");
...
__name(parseRangeDate, "parseRangeDate");
async function handleCrawlRange(request, env) {
  if (env.IMPORT_TOKEN) {
    const token = request.headers.get("x-import-token");
    if (!token || token !== env.IMPORT_TOKEN) {
      return jsonResponse({ error: "unauthorized" }, { status: 401 });
  ...
  for (let pageIndex = 0; pageIndex < maxPages; pageIndex += 1) {
    ...
    for (const item of items) {
      const id = extractDictamenId2(item);
      if (!id) continue;
...
__name(handleCrawlRange, "handleCrawlRange");
...
async function handleCompareCanonical(request, env) {
  if (env.IMPORT_TOKEN) {
    const token = request.headers.get("x-import-token");
    if (!token || token !== env.IMPORT_TOKEN) {
      return jsonResponse({ error: "unauthorized" }, { status: 401 });
...
__name(handleCompareCanonical, "handleCompareCanonical");
...
async function handleMistralDebug(request, env) {
  if (env.IMPORT_TOKEN) {
    const token = request.headers.get("x-import-token");
    if (!token || token !== env.IMPORT_TOKEN) {
      return jsonResponse({ error: "unauthorized" }, { status: 401 });
...
__name(handleMistralDebug, "handleMistralDebug");
async function handleRecoverFuentes(request, env) {
  if (env.IMPORT_TOKEN) {
    const token = request.headers.get("x-import-token");
    if (!token || token !== env.IMPORT_TOKEN) {
      return jsonResponse({ error: "unauthorized" }, { status: 401 });
...
__name(handleRecoverFuentes, "handleRecoverFuentes");
async function handleEdgeCases(request, env) {
  if (env.IMPORT_TOKEN) {
    const token = request.headers.get("x-import-token");
    if (!token || token !== env.IMPORT_TOKEN) {
      return jsonResponse({ error: "unauthorized" }, { status: 401 });
...
__name(handleEdgeCases, "handleEdgeCases");
async function runEnrich(env, dictamenId, enqueueVectorize) {
  const runId = await startRun(env.DB, "enrich", { dictamenId });
  const allowed = await consume(env.STATE_KV, env.DAILY_QUOTA, 1);
  if (!allowed) {
...
__name(runEnrich, "runEnrich");
async function runVectorize(env, dictamenId) {
...
__name(runVectorize, "runVectorize");
async function runFuentesLegales(env, dictamenId) {
...
__name(runFuentesLegales, "runFuentesLegales");
async function handleQueueMessage(message, env) {
  switch (message.type) {
    case "crawl": {
      ...
      const ids = page.items.map((item) => extractDictamenId2(item)).filter((id) => Boolean(id));
      const existing = await getExistingDictamenIds(env.DB, ids);
      const canonicals = await getDictamenCanonicals(env.DB, ids);
...
__name(handleQueueMessage, "handleQueueMessage");
var index_default = {
  async fetch(request, env) {
    const url = new URL(request.url);
    if (url.pathname === "/health") {
      return jsonResponse({ ok: true, time: (/* @__PURE__ */ new Date()).toISOString() });
    }
  ...
  async scheduled(_controller, env) {
    ...
    const allowed = await canConsume(env.STATE_KV, env.DAILY_QUOTA, 1);
    if (!allowed) return;
